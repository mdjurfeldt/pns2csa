...
\section{The Connection Generator API}\label{sec:conn_gen}
...
section enables the simulators use external connection generating
libraries. The benchmarks in Section \ref{sec:benchmarks} use the
interface to couple NEST to the connection generating libraries
...
it can also be used when all network connections are given or in cases
...
Assume that a mask is specifying connectivity for a single population
...
1)\}$ would be appropriate. Such masks do not only contain information
...
connects the elements with each other. NEST provides two basic ways
for creating connectivity.
...
For such cases, the connection functions \verb|ConvergentConnect| and
\verb|DivergentConnect| can be used to create multiple connections
with a single call. In addition, random variants for both functions
exist to support the user in creating networks on the basis of
knowledge of connectivity statistics. However, random connection
parameters (e.g. \emph{weight} or \emph{delay}) need to be specified
...
\subsubsection{Connection generating libraries}

As detailed above, NEST has several methods for connecting neurons
into a network built-in. However, while the native routines scale very
well \citep{Helias12_26}, they are only suitable for creating simple
patterns such as convergent/divergent connectivity without using them
in an iterative manner. On the other hand, the topology module
\citep{Plesser_13} allows to form more complex structures, but
requires neurons to be organized in a layers. This has some overhead,
which is also reflected in the performance and scaling.

To support the \verb|ConnectionGenerator| interface (see
Section~\ref{sec:conn_gen}) in NEST and thus make more connection
generating libraries available to the users, we created the
...
Generator interface in NEST, although that itself uses indices
starting from zero (see Section~\label{sec:cgint}). Our new Connection
Generator interface for NEST consists od the following functions:
...
When PyNEST is used, the \verb|ConnectionGenerator| object can be
created in Python using the Python wrapper of \verb|libcsa|. In the
call to \verb|CGConnect|, a pointer to the underlying
...
until there are no connections anymore. The connections are internally
established one by one by calling NEST's basic
\verb|Network::connect()| function on the C++ level.
