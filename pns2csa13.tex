...
\subsection{PYNN Connection Generators}
...
\ref{sec:conn_gen_pynn}). These benchmarks were using PyNN as available , In particular, the following combinations
were tested:
...
\caption{Benchmark results for the use of CSA in PyNN and NEST
  ...
  legends, slope is the mean slope of the data shown. \textbf{A} shows
  the run time for connecting a network using CSAs \emph{random} mask
  with a probality of 0.1 for different numbers of neurons. \textbf{B}
  shows the same as in A, but using CSAs \emph{oneToOne}
  mask. \textbf{C} shows the memory consumption of the Python process
  after connecting the network described in A. \textbf{D} shows the
  results of a strong scaling benchmark for different numbers of MPI
  processes. The network contains 1,000 neurons per process and is
  connected as the network in A.}\label{fig:pynn_benchmarks}
\end{figure}
