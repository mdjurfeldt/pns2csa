...
connectivity. By providing connectivity-generating software, with
...
between a source population of neurons to a target population.
...
and constitutes an abstraction barrier between the actual elements of a
...
source and target populations. Which source and target index sets that
are relevant to iterate over for the populations at hand are specified
through the interface as a \emph{mask}.  In the case of a parallel
simulator, a specific mask is given per rank.  Typically, the source
...
section enables the simulators use external connectivity-generating
...
more connectivity-generating libraries available to the user, we created
...
flexibly choose a connectivity-generating library independently of the
