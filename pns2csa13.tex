...
either in the simulator or connection generating library source trees,
it will be duplicated.  As new versions of the interface are
developed, the situation will quickly cease to be maintainable.  In
fact, the \verb|ConnectionGenerator| is unusual in that it is a
symmetric abstraction barrier: it both allows the use of any library
providing the API from a given simulator \emph{and} allows the same
library to be used from any simulator supporting the API.
...
(\url{http://software.incf.org/software/libneurosim}).  This library
currently consists of two main components.
