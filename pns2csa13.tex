...
connection-generating functions as a task to the simulator
developers. Especially in a parallel environment, this is not a
trivial pursuit. One way to ease this problem is the use of general
...
PyNN \citep[\url{http://www.neuralensemble.org/PyNN};][]{Davison09} is a
...
\citep[\url{http://www.nest-initiative.org};][]{Gewaltig_07_11204}. It is
...
\tbw{Should we add a figure here? Maybe the ones
  from\\ http://nest-initiative.org/index.php/Connection\_Management?\\ Mikael:
  I think we should first write all text and then prioritize what the
  most important needed figures are.}
...
gain of this will be demonstrated in section \ref{sec:algebra}.
...
\subsection{An algebra for connectivity structure}\label{sec:algebra}
...
\caption{Sequence diagram showing the function calls during the use of
  the ConnectionGenerator interface in NEST. A user first creates a
  \verb|ConnectionGenerator| object \emph{cg}. She then calls
  PyNEST's \verb|CGConnect()| function. \textcircled{\footnotesize
    1} The function \verb|cg_get_ranges()| returns all contiguous
  ...
  kernel iterates the ConnectionGenerator by calling \verb|next()|
  until there are no more connections. For each connection it
  receives, it creates the connection by calling
  \verb|Network::connect()|. If a non-empty \emph{param\_map} was
  given to \verb|CGConnect|, the connection's weight and delay are
...
\tbw{To be written by Jochen}

%----------------------------------------------------------------------

\section{Benchmarks}\label{sec:benchmarks}

We ran a series of benchmarks in order to assess the performance of and
compare two implementations of CSA (see section
\ref{sec:csa}) and the two implementations of the CSAConnector
...
\item \textbf{PyNN, csa} This used PyNN's original CSAConnector that
  ...
  implementation (retrieved from the INCF software center via SVN).
  ...
  Python level. It connects by sending one ConvergentConnect call per
...
\item \textbf{nest, csa} This used my new CSAConnector for the NEST
  ...
  \url{https://github.com/jougs/PyNN}) and the Python CSA
...
routines work either on single GISs or on lists of GIDs. When calling
