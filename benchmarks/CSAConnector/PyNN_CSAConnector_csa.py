...
if len(argv) != 2:
    print "usage: PyNN_CSAConnector_csa.py <num_neurons>"
    exit()
...
print "PyNN.CSAConnector (oneToOne, csa, n=%i): %f" % (n, time() - start)
...
connector = CSAConnector(cset)
proj = Projection(pop, pop, connector)
print "PyNN.CSAConnector (random(0.1), csa, n=%i): %f" % (n, time() - start)
