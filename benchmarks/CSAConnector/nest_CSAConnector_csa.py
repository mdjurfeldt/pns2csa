...
if len(argv) != 2:
    print "usage: nest_CSAConnector_csa.py <num_neurons>"
    exit()
...
print "nest.CSAConnector (oneToOne, csa, n=%i): %f" % (n, time() - start)
...
connector = CSAConnector(cset)
proj = Projection(pop, pop, connector)
print "nest.CSAConnector (random(0.1), csa, n=%i): %f" % (n, time() - start)
