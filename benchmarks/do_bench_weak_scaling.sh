...
for np in 1 2 4 6 12 24 48; do
  ...
  mpirun -np $np python PyNN_FixedProbabilityConnector_scaling.py $nn $np > data/PyNN_FixedProbabilityConnector_weak_scaling_$np.log
  mpirun -np $np python nest_RandomConvergentConnect_scaling.py $nn $np > data/nest_RandomConvergentConnect_weak_scaling_$np.log
